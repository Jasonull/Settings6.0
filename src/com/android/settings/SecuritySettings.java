...
public class SecuritySettings extends SettingsPreferenceFragment
    ...
    private static final String KEY_PRIVACY_UNLOCK = "privacy_unlock";  //add by zhangwenqiang @20150701

    // These switch preferences need special handling since they're not all stored in Settings.
    private static final String SWITCH_PREFERENCE_KEYS[] = { KEY_LOCK_AFTER_TIMEOUT,
            KEY_LOCK_ENABLED, KEY_VISIBLE_PATTERN, KEY_BIOMETRIC_WEAK_LIVELINESS,
            KEY_POWER_INSTANTLY_LOCKS, KEY_SHOW_PASSWORD, KEY_TOGGLE_INSTALL_APPLICATIONS, KEY_PRIVACY_UNLOCK };
    ...
    //add by zhangwenqiang @20150701
    private SwitchPreference mPrivacyUnlock;
    ...
    private SwitchPreference  mGoHomeAfterLock;
    //added of by zhaoxiaoxiang
        ...
        if (!lockPatternUtils.isSecure()) {
            ...
            switch (lockPatternUtils.getKeyguardStoredPasswordQuality()) {
                ...
                case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC_COMPLEX:
                	//HCT: xuejin fingerprint settings
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        if (!getResources().getBoolean(R.bool.hct_remove_encrypted) && isEMMC && mIsPrimary) {//remove security by chaixincheng
            if (LockPatternUtils.isDeviceEncryptionEnabled()) {
                // The device is currently encrypted.
                addPreferencesFromResource(R.xml.security_settings_encrypted);
            } else {
                // This device supports encryption but isn't encryptedisEMMC && !isLowRAM && mIsPrimary.
        ...
        if (mLockAfter != null) {
       // add by hct-luoxiaosong for remove lock after start
       if(getResources().getBoolean(R.bool.hct_config_remove_lockafter)){
       	securityCategory.removePreference(mLockAfter);	
       }else{
        //add by hct-luoxiaosong for remove lock after end
	...
	//}
        ...
        //added by zhaoxiaoxiang for goto home after locked
        mGoHomeAfterLock=(SwitchPreference)root.findPreference(
        		"go_home_after_lock");
		if(mGoHomeAfterLock != null){
        	mGoHomeAfterLock.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
			@Override
			public boolean onPreferenceClick(Preference preference) {
				// TODO Auto-generated method stub
				if(mGoHomeAfterLock.isChecked()){
					Settings.System.putInt(getContentResolver(), "go_home_after_lock", 1);
				}else{
					Settings.System.putInt(getContentResolver(), "go_home_after_lock", 0);
				}
				return true;
			}
		});
        ...
        try {
        	mGoHomeAfterLock.setChecked(Settings.System.getInt(getContentResolver(), "go_home_after_lock")==1);
        	if(Settings.System.getInt(getContentResolver(), "show_go_home_menu")==1){
        		removePreference("go_home_after_lock");
        	}
		} catch (Exception e) {
        ...
        //add by zhangwenqiang @20150701
        mPrivacyUnlock = (SwitchPreference) findPreference(KEY_PRIVACY_UNLOCK);
        if (mPrivacyUnlock != null) {
            mPrivacyUnlock.setChecked(android.os.SystemProperties.get("persist.sys.privacy_unlock").equals("1"));
        }

        if (mPrivacyUnlock != null && !getResources().getBoolean(R.bool.config_show_hct_privacy_unlock)) {
            securityCategory.removePreference(root.findPreference(KEY_PRIVACY_UNLOCK));
        }
        //end by zhangwenqiang @20150701
        ...
        if (advancedCategory != null) {
            ...
            //add by zhangwenqiang @20151116 start 
            if (root.findPreference(KEY_SCREEN_PINNING) != null
                  && !SystemProperties.get("ro.hct_cts_modify").equals("1")) {
                advancedCategory.removePreference(root.findPreference(KEY_SCREEN_PINNING));
            }
            //add by zhangwenqiang @20151116 end 
        }
        ...
        Preference credentialInstall = root.findPreference(KEY_CREDENTIALS_INSTALL);
	...
	//add by hct-luoxiaosong for modify lock after default values start
        long currentTimeout = Settings.Secure.getLong(getContentResolver(),
                Settings.Secure.LOCK_SCREEN_LOCK_AFTER_TIMEOUT, getResources().getInteger(R.integer.default_lock_after_values));
	//add by hct-luoxiaosong for modify lock after default values end
    ...
    public boolean onPreferenceChange(Preference preference, Object value) {
        ...
        } else if (KEY_PRIVACY_UNLOCK.equals(key)) {
            //add by zhangwenqiang @20150701
            if (!mPrivacyUnlock.isChecked()) {
                mPrivacyUnlock.setChecked(true);
                android.os.SystemProperties.set("persist.sys.privacy_unlock","1");
            } else {
                mPrivacyUnlock.setChecked(false);
                android.os.SystemProperties.set("persist.sys.privacy_unlock","0");
            }
            //end by zhangwenqiang @20150701
        }
        return result;
