...
final class CanBeOnSdCardChecker {
    ...
    boolean check(ApplicationInfo info) {
        ...
        if ((info.flags & ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0) {
            canBe = true;
        /// M: [ALPS00270065] @{
        } else if ((info.flagsEx & ApplicationInfo.FLAG_EX_OPERATOR) == 0) {
        /// @}
            ...
            if (mFilter == FILTER_APPS_SDCARD) {
                ...
                if (mContainerService != null) {
                    try {
                        ...
                        mTotalStorage = stats[0];
//add by hct-zhangcy for modify ram and rom
                       int rom = mOwner.getActivity().getResources().getInteger(R.integer.default_customer_rom_values);
                       int rom_point = mOwner.getActivity().getResources().getInteger(R.integer.default_customer_rom_values_point);
                       if(rom > 0){
                          mTotalStorage = rom*1024*1024*1024L/rom_point;
                       }
//add by hct-zhangcy for modify ram and rom end
                        mFreeStorage = stats[1];
//add by hct-zhangcy for modify ram and rom
                       int freerom = mOwner.getActivity().getResources().getInteger(R.integer.default_customer_rom_free_values);
                       int freerom_point = mOwner.getActivity().getResources().getInteger(R.integer.default_customer_rom_free_values_point);
                       if(freerom > 0){
                          if(mFreeStorage < 50*1024*1024L){
                             mFreeStorage = mFreeStorage;
                          }else{
                              mFreeStorage = mFreeStorage + freerom*1024*1024*1024L/freerom_point;
                          }
                       }
                    } catch (RemoteException e) {
                        Log.w(TAG, "Problem in container service", e);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!Environment.isExternalStorageEmulated()
                && !(FeatureOption.MTK_2SDCARD_SWAP && !UtilsExt.isExSdcardInserted())) {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        menu.add(0, SORT_ORDER_SIZE, 2, R.string.sort_order_size)
                ...
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        // M : long string solution
        menu.add(0, SHOW_RUNNING_SERVICES, 3,  R.string.show_running_services)
                .setIcon(R.drawable.ic_menu_apps_show_service)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
        menu.add(0, SHOW_BACKGROUND_PROCESSES, 3,  R.string.show_background_processes)
                .setIcon(R.drawable.ic_menu_apps_show_cache)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
