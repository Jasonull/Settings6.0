...
public class BatteryHistoryChart extends View {
    static final boolean DEBUG = false; // modify by hct lishuo 2015.12.31
    ...
    void setStats(BatteryStats stats, Intent broadcast) {
        ...
        if (mBatteryBroadcast.getIntExtra(BatteryManager.EXTRA_PLUGGED, 0) == 0) {
            ...
            if (drainTime > 0) {
                ...
                String timeString = Formatter.formatShortElapsedTime(getContext(),
                        drainTime / 1000);
						//hct macong  battery view error 2015.10.6
						if(getContext().getResources().getBoolean(R.bool.hct_config_show_batteryleavel_inbattery)){
                ...
                mChargeLabelString = getContext().getResources().getString(
					...
					//hct macong  battery view error 2015.10.6 end
            } else {
                mChargeLabelString = batteryPercentString;
            }
        } else {
            ...
            while (stats.getNextHistoryLocked(rec)) {
                ...
                if (rec.cmd == HistoryItem.CMD_CURRENT_TIME
                    ...
                    if (mStartWallTime == 0) {
                        ...
                        if (DEBUG)
                            Log.d(TAG, "lastWallTime: " + lastWallTime
                                    + " lastRealtime: " + lastRealtime
                                    + " mHistStart: " + mHistStart);
                    }
                }
                if (rec.isDeltaData()) {
        ...
        if (DEBUG)
            Log.d(TAG, "mEndDataWallTime: " + mEndDataWallTime
                    + " remainingTimeUs: " + remainingTimeUs);
    ...
    void finishPaths(int w, int h, int levelh, int startX, int y, Path curLevelPath,
        ...
        if (DEBUG) Log.d(TAG, "finishPaths w = " + w + ",h = " + h + ",levelh = " + levelh
                + ",startX = " + startX + ",y = " + y + ",lastX = " + lastX);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mEndDataWallTime > mStartWallTime && mStats.startIteratingHistoryLocked()) {
            ...
            while (mStats.getNextHistoryLocked(rec) && i < N) {
                if (rec.isDeltaData()) {
                    ...
                    if (DEBUG) Log.d(TAG, "isDeltaData curWalltime = " + curWalltime + ",lastRealtime = "
                            + lastRealtime + ",x = " + x);
                    if (x < 0) {
                        x = 0;
                    }
                    ...
                    if (DEBUG) Log.d(TAG, "isDeltaData y = "+ y);
                    if (lastX != x) {
                        ...
                        if (lastY != y) {
                            ...
                            if (DEBUG) Log.d(TAG, "isDeltaData value = "+ value);
                    ...
                    if (DEBUG) Log.d(TAG, "rec.cmd = "+ rec.cmd + ",lastWalltime = " + lastWalltime);
                    if (rec.cmd == HistoryItem.CMD_CURRENT_TIME
                            || rec.cmd == HistoryItem.CMD_RESET) {
                        ...
                        lastRealtime = rec.time;
                        if (DEBUG) Log.d(TAG, "curWalltime = " + curWalltime + ",lastRealtime = " + lastRealtime);
                    }
       ...
       Log.d(TAG, "while done lastY = " + lastY + ",lastX = " + lastX);
        if (lastY < 0 || lastX < 0) {
            // Didn't get any data...
            x = lastX = mLevelLeft;
    ...
    void drawChart(Canvas canvas, int width, int height) {
        ...
        if (mTimeLabels.size() > 1) {
            ...
            if (DEBUG) Log.d(TAG, "Drawing mDurationString: " + mDurationString);
            int y = mLevelBottom - mTextAscent + (mThinLineWidth*4);
            mTextPaint.setTextAlign(Paint.Align.LEFT);
