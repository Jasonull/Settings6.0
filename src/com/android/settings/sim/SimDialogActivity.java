...
import android.telephony.TelephonyManager;
import android.view.WindowManager; //chenyichong add.
...
public class SimDialogActivity extends Activity {
    ...
    public static final int HCT_DATA_PICK = 4; //chenyichong add.
    public static final int DATA_PICK_NO_RET = 5; //chenyichong add.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            ...
            switch (dialogType) {
                case DATA_PICK:
                case DATA_PICK_NO_RET: //chenyichong add.
                case CALLS_PICK:
                case SMS_PICK:
                case HCT_DATA_PICK: //chenyichong add.
    ...
    private void displayPreferredDialog(final int slotId) {
        ...
        Log.d("Bob.chen", "displayPreferredDialog. slotId = . " + slotId);

        if (sir != null) {
//chenyichong modify begin. fix bug: ALPS02023572
            if(!getResources().getBoolean(R.bool.config_support_poweron_onesim_ignore_dialog)){
            ...
            //chenyichong modify end. fix bug: ALPS02023572
    ...
    public Dialog createDialog(final Context context, final int id) {
                    ...
                    public void onClick(DialogInterface dialog, int value) {
                        ...
                        switch (id) {
                            ...
                            case HCT_DATA_PICK:
                                ...
                                if (!mExt.switchDefaultDataSub(context, sir.getSubscriptionId())) {
                                    setDefaultDataSubId(context, sir.getSubscriptionId());
                                    setMobileDataEnabled(context, sir.getSubscriptionId(), true);
                                    disableDataForOtherSubscriptions(context, subInfoList, sir);
                                }
                                break;
                            //chenyichong add end.
                            default:
        ...
        //chenyichong modify for fix bug: if user press backkey, the dialog will disappear, and will not hint again.
        if(id == DATA_PICK_NO_RET){
            dialog.setOnKeyListener(new Dialog.OnKeyListener() {
                @Override
                public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
                    if (keyCode == KeyEvent.KEYCODE_BACK) {
                        //finish(); //chenyichong modify for fix bug: if user press backkey, the dialog will disappear, and will not hint again.
                    }
                    return true;
                }
                });
        ...
        */
        ...
        //chenyichong modify for fix bug end.
...
//chenyichong add begin.
    private void setMobileDataEnabled(final Context context, int subId, boolean enabled) {
        ...
        Log.d("Bob.chen", "SimDialogActivity. setMobileDataEnabled(). subId = " + subId + ". enabled = " + enabled);
        TelephonyManager.from(context).setDataEnabled(subId, enabled);
        //mMobileDataEnabled.put(String.valueOf(subId), enabled);
        //updatePolicy(false);
    }

    private void disableDataForOtherSubscriptions(final Context context, final List<SubscriptionInfo>  mSubInfoList, SubscriptionInfo currentSir) {
