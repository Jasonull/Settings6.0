...
public class StorageMeasurement {
    ...
    private class MeasurementHandler extends Handler {
        ...
        private boolean isSupportHotSwap = false;
        ...
        public MeasurementHandler(Context context, Looper looper) {
            ...
            isSupportHotSwap = context.getResources().getBoolean(R.bool.config_sdcard_support_swap_hot);
        }
		...
		private boolean isHaveExternalSDCard(){
			  ...
			  try {
			       sStorageManager = new StorageManager(context.getContentResolver(),handler.getLooper());
			  } catch (Exception e) {
			      e.printStackTrace();
			  }
		       }
        ...
        private void measureApproximateStorage(IMediaContainerService imcs) {
            ...
            try {
                ...
                mTotalSize = stats[0];
		        final Context context = (mContext != null) ? mContext.get() : null;
                boolean isSupportHotSwap = context.getResources().getBoolean(R.bool.config_sdcard_support_swap_hot);
                int internal = context.getResources().getInteger(R.integer.config_internal_storage);
                int internal_point = context.getResources().getInteger(R.integer.config_internal_storage_point);
                int external = context.getResources().getInteger(R.integer.config_external_storage);
                int external_point = context.getResources().getInteger(R.integer.config_external_storage_point);
		        if(external > 0 || internal > 0){
                   if(internal > 0 && mVolume == null){
                      mTotalSize = internal*1024*1024*1024L/internal_point;
                   }
                   if(mVolume != null && external > 0){
		               if(isSupportHotSwap){
                          if(isHaveExternalSDCard()){
		                     if(path.equals("/storage/sdcard1")){
				                 mTotalSize = external*1024*1024*1024L/external_point;                            
                             }
                           }else{
                ...
                mAvailSize = stats[1];
                int internal_free = context.getResources().getInteger(R.integer.config_internal_storage_free);
                int internal_free_point = context.getResources().getInteger(R.integer.config_internal_storage_free_point);
                int external_free = context.getResources().getInteger(R.integer.config_external_storage_free);
                int external_free_point = context.getResources().getInteger(R.integer.config_external_storage_free_point);
                if(external_free > 0 || internal_free > 0){
                   if(internal_free > 0 && mVolume == null){
                      mAvailSize = mAvailSize + internal_free*1024*1024*1024L/internal_free_point;
                   }
                   if(mVolume != null && external > 0){
		               if(isSupportHotSwap){
                          if(isHaveExternalSDCard()){
		                     if(path.equals("/storage/sdcard1")){
                                mAvailSize = mAvailSize + external_free*1024*1024*1024L/external_free_point;                           
                             }
                           }else{
		                     if(path.equals("/storage/sdcard0")){
                                mAvailSize = mAvailSize + external_free*1024*1024*1024L/external_free_point;                           
    ...
    /** M: CR ALPS01284632, Add call back to update adapter. @{ */
    ...
    public interface ICallBack {
        public void callBack(List<FileInfo> fileInfoForMisc);
    }
    /** @} */
    ...
    private long measureMisc(IMediaContainerService imcs, File dir) {
        ...
        for (File file : files) {
            ...
            /** M: CR ALPS01284632, Update adapter every time we scanned
             *     10 files or folder. @{ */
            scannedCount++;
            if (scannedCount == 10) {
                scannedCount = 0;
                if (mICallBack != null) {
                    mICallBack.callBack(mFileInfoForMisc);
                }
           }
           /** @} */
        }

        /** M: CR ALPS01284632, Update adapter when scanned result is less
         *     than 10 or not a multiple of 10. @{ */
        if (mICallBack != null) {
            mICallBack.callBack(mFileInfoForMisc);
        }
        /** @} */
