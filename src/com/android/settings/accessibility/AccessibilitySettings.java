...
public class AccessibilitySettings extends SettingsPreferenceFragment implements DialogCreatable,
	...
	//add by xubinbin Silent the call by turn the phone 2015-7-18
    private static final String TOGGLE_TURN_SILENCE_PREFERENCE =
            "toggle_turn_silence_preference";
   ...
   //add by macong for fastcamera 2015.9.9
    private static final String FASTCAMERA_SETTING_PREFERENCE = "fastcamera_setting";
   //add by macong for fastcamera end   2015.9.9
	...
	//add by xubinbin Silent the call by turn the phone 2015-7-18
    private SwitchPreference mToggleTurnSilencePreference;
   ...
   //add by macong for fastcamera 2015.9.9
    private SwitchPreference mFastCameraSetting;
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (mSelectLongPressTimeoutPreference == preference) {
            ...
            Settings.System.putInt(getContentResolver(), Settings.System.IPO_SETTING,
                ...
                return true;
             /** @} */
        } else if(mFastCameraSetting == preference){ //add by macong for fastcamera
          boolean flag = (Boolean)newValue;
          if(flag)
             android.os.SystemProperties.set("persist.sys.fastcamera","true");
          else
            android.os.SystemProperties.set("persist.sys.fastcamera","false");
          return true;
        }
        return false;
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (mToggleLargeTextPreference == preference) {
            ...
            return true;
		//add by xubinbin Silent the call by turn the phone 2015-7-18
        } else if (mToggleTurnSilencePreference == preference) {
        SystemProperties.set(PROPERTIES_TURN_SILENCE, mToggleTurnSilencePreference.isChecked() ? "true" : "false");
        String act = "hct.intent.stop.silentservice";
        if(mToggleTurnSilencePreference.isChecked()){
    ...
    private void initializeAllPreferences() {
        ...
        // Display inversion.//remove toggle inversion by chaixincheng
        ...
        if(getResources().getBoolean(R.bool.config_remove_hightext_menu)){
            mSystemsCategory.removePreference(mToggleHighTextContrastPreference);
        }
        if(getResources().getBoolean(R.bool.config_remove_inversion_menu)){
            ((PreferenceCategory)findPreference("display_category")).removePreference(mToggleInversionPreference);
        }
         //liuwenbo add start 
	 if(getResources().getBoolean(R.bool.config_remove_large_text_menu)){
	...
	//add by xubinbin Silent the call by turn the phone 2015-7-18
        mToggleTurnSilencePreference =
                (SwitchPreference) findPreference(TOGGLE_TURN_SILENCE_PREFERENCE);
	if(mToggleTurnSilencePreference != null){
		if(getResources().getBoolean(R.bool.config_accessibility_remove_turnsilence)){
		mSystemsCategory.removePreference(mToggleTurnSilencePreference);
		}
		else {
        mToggleTurnSilencePreference.setChecked(SystemProperties.getBoolean(PROPERTIES_TURN_SILENCE, false));
		}
        }
	//add by xubinbin Silent the call by turn the phone 2015-7-18
        
        //liuwenbo add start 2015-9-14
          if(mToggleSpeakPasswordPreference != null){
            if(getResources().getBoolean(R.bool.config_remove_speak_password_menu)){
            mSystemsCategory.removePreference(mToggleSpeakPasswordPreference);
            }
          }
       	//liuwenbo add end 2015-9-14
	...
	//remove GlobalGesture by chaixincheng
        if (true){//(!KeyCharacterMap.deviceHasKey(KeyEvent.KEYCODE_POWER) 
		...
		//add by chaixincheng
	//	if(getResources().getBoolean(R.bool.config_remove_captions)){
			mSystemsCategory.removePreference(mCaptioningPreferenceScreen);
	//	}

	//zhangjunjun add begin @20150918
        if (!getResources().getBoolean(R.bool.hct_config_show_globalGesturePreference)) {
            mSystemsCategory.removePreference(mGlobalGesturePreferenceScreen);
        }
        
        if (!getResources().getBoolean(R.bool.hct_config_show_LargeTextPreference)) {
            mSystemsCategory.removePreference(mToggleLargeTextPreference);
        }
        //zhangjunjun add end @20150918
	...
	//add by macong for fastcamera 2015.9.9	
        mFastCameraSetting = (SwitchPreference) findPreference(FASTCAMERA_SETTING_PREFERENCE);
        if(mFastCameraSetting != null){
            mFastCameraSetting.setOnPreferenceChangeListener(this);
            if(!android.os.SystemProperties.get("ro.hct_fastcamera").equals("1")){
                mSystemsCategory.removePreference(mFastCameraSetting);
            }
        }
		//add by macong for fastcamera 2015.9.9
    }
    ...
    private void updateServicesPreferences() {
        ...
        for (int i = 0, count = installedServices.size(); i < count; ++i) {
            ...
            //chenyichong modify for hide accessibility service.
            if (!getResources().getBoolean(R.bool.config_hide_accessibility_service)) {
    ...
    private void updateSystemPreferences() {
        ...
        // If the quick setting is enabled, the preference MUST be enabled.//remove toggle inversion by chaixincheng
        ...
        /// @} end
       //add by macong for fastcamera 2015.9.9
        boolean fastcameraOnoff = android.os.SystemProperties.getBoolean("persist.sys.fastcamera", false);
        if(mFastCameraSetting != null){
           mFastCameraSetting.setChecked(fastcameraOnoff);
        }
       //add by macong for fastcamera end       
    }
