...
import android.util.Log;  //add  by weixiaohui.hct for AudioProfile setting
import android.preference.ListPreference;  //add by weixiaohui.hct for virbate time settings  
import android.os.SystemProperties;//add by weixiaohui.hct for virbate time settings
...
public class Editprofile extends SettingsPreferenceFragment {
    ...
    public static final String KEY_CALL_VOLUME = "call_volume";   //add  by weixiaohui.hct for AudioProfile setting
	//yangguodong add for pocket mode 20141126
    public final static String KEY_POCKET_MODE = "pocket_mode";
    private static final String TAG = "AudioProfile/EditProfile";
	public static final String KEY_VIBRATE_TIME_SETTINGS = "vibrate_time_settings"; //add by weixiaohui.hct for virbate time settings
	...
	//yangguodong add for pocket mode 20141126	
    private TwoStatePreference mPocketMode;
    private ListPreference mVirbatePreference; //add by weixiaohui.hct for virbate time settings
    ...
    private boolean mSnowMediaVolume;
    private boolean mSnowCallVolume;
    ...
    private void initPreference() {
		...
		//+add by weixiaohui.hct for virbate time settings 
				initVirbateTimeSettings(); 
			   if(SystemProperties.get("ro.hct_vibrate_time_setting").equals("1")){ 			
				  parent.removePreference(parent.findPreference(KEY_VIBRATE));
				} else{
				  parent.removePreference(parent.findPreference(KEY_VIBRATE_TIME_SETTINGS));
				}	
	   //-add by weixiaohui.hct for virbate time settings		
	   mSnowMediaVolume = mContext.getResources().getBoolean(R.bool.hct_show_meida_volume);
	   mSnowCallVolume = mContext.getResources().getBoolean(R.bool.hct_show_call_volume);

		if(mSnowMediaVolume&&!mSnowCallVolume){			
			parent.removePreference(parent.findPreference(KEY_CALL_VOLUME));
		}else if(mSnowCallVolume&&!mSnowMediaVolume){			
			parent.removePreference(parent.findPreference(KEY_MEDIA_VOLUME));
		}else if(mSnowMediaVolume&&mSnowMediaVolume){
		   //do noting
		}else{			
			parent.removePreference(parent.findPreference(KEY_CALL_VOLUME));
			parent.removePreference(parent.findPreference(KEY_MEDIA_VOLUME));
	...
	//+add by weixiaohui.hct for virbate time settings  
		private void initVirbateTimeSettings(){
				mVirbatePreference = (ListPreference) findPreference(KEY_VIBRATE_TIME_SETTINGS);	  
				final long currentTimeValue = getVirbateTimeValue();	
				mVirbatePreference.setValue(String.valueOf(currentTimeValue));		 
				disableUnusableVirbateTimes(mVirbatePreference);
				setListPreferenceListener(KEY_VIBRATE_TIME_SETTINGS,mVirbatePreference);		  
				updateVirbateTimePreferenceDescription(currentTimeValue);	
			}
		   private int getVirbateTimeValue() {
				int currentValue = Settings.System.getInt(getActivity().getContentResolver(), "virbate_time_setting",0);
				int bestMatch = 0;
				int timeout = 0;
				final CharSequence[] valuesTimeout = mVirbatePreference.getEntryValues();
			...
			private void updateVirbateTimePreferenceDescription(long currentTimeout) {
				ListPreference preference = mVirbatePreference;
				String summary;
				if (currentTimeout < 0) {
					summary = "";
				} else {
					...
					if (entries.length != 0) {
						summary = preference.getContext().getString(
								R.string.vibrate_timeout_summary, entries[best]);
					} else {
						summary = "";
					}
			...
			private void disableUnusableVirbateTimes(ListPreference mVirbatePreference) {
		
				final long maxTimeout = 7000;
				if (maxTimeout == 0) {
					return; 
				}
				final CharSequence[] entries = mVirbatePreference.getEntries();
				final CharSequence[] values = mVirbatePreference.getEntryValues();
				...
				if (revisedEntries.size() != entries.length || revisedValues.size() != values.length) {
					final int userPreference = Integer.parseInt(mVirbatePreference.getValue());
					mVirbatePreference.setEntries(
							revisedEntries.toArray(new CharSequence[revisedEntries.size()]));
					mVirbatePreference.setEntryValues(
							revisedValues.toArray(new CharSequence[revisedValues.size()]));
					if (userPreference <= maxTimeout) {
						mVirbatePreference.setValue(String.valueOf(userPreference));
					} else if (revisedValues.size() > 0
							&& Long.parseLong(revisedValues.get(revisedValues.size() - 1).toString())
							== maxTimeout) {
						mVirbatePreference.setValue(String.valueOf(maxTimeout));
					} else {
		
					}
				}
				mVirbatePreference.setEnabled(revisedEntries.size() > 0);
			}
		//-add by weixiaohui.hct for virbate time settings
    ...
    private void initVolume(PreferenceScreen parent) {
        ...
        initVolumePreference(KEY_MEDIA_VOLUME, AudioManager.STREAM_MUSIC);  //add  by weixiaohui.hct for AudioProfile setting
        initVolumePreference(KEY_CALL_VOLUME, AudioManager.STREAM_VOICE_CALL);//add  by weixiaohui.hct for AudioProfile setting
    }
...
PreferenceScreen parent = getPreferenceScreen();
        ...
        mHapticFeedback = (TwoStatePreference) findPreference(KEY_HAPTIC_FEEDBACK);
			//yangguodong add for pocket mode 20141126
	 mPocketMode = (TwoStatePreference) findPreference(KEY_POCKET_MODE);
	 if (mPocketMode != null) {
		if (getResources().getBoolean(R.bool.config_pocket_mode_enable)) {
			//Temporarily by persist method   later maybe add to audioprofile system
			String defaultPocketMode = android.os.SystemProperties.get("persist.sys.hctpocketmode.state", "none");
			if(!defaultPocketMode.equals("1")) {
				mPocketMode.setChecked(false);
			}
		} else {
			parent.removePreference(mPocketMode);
		}
	}
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        }else if ((preference.getKey()).equals(KEY_POCKET_MODE)) {
        	setPocketMode(mPocketMode.isChecked());
        }

        return super.onPreferenceTreeClick(preferenceScreen, preference);
    }
    //yangguodong add for pocket mode 20141126	
    private void setPocketMode(boolean checked){
        if(checked) {
            android.os.SystemProperties.set("persist.sys.hctpocketmode.state", "1");
        }else {
            android.os.SystemProperties.set("persist.sys.hctpocketmode.state", "0");
        }
    }
	...
	//+add by weixiaohui.hct for virbate time settings	 
		   private void setListPreferenceListener(final String preferenceType, ListPreference p) {
				p.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
		
					@Override
				  public boolean onPreferenceChange(Preference preference, Object objValue) {
                   int value = Integer.parseInt((String) objValue);                 
                   long [] styele1 = {500,400,500,400};    
                   long [] styele2 = {200,1200,200,1200};  
                   long [] styele3 = {200,300,200,300};   
                   Settings.System.putInt(getContentResolver(), "virbate_time_setting", value);
                   updateVirbateTimePreferenceDescription(value);
					 if(value == 1){
					 	   mVibrator.vibrate(styele1,-1);
					 	} else if (value == 2){					 	
						   mVibrator.vibrate(styele2,-1);
					 	}else if (value == 3){					 	
						   mVibrator.vibrate(styele3,-1);
					 	}
						 return true;
					}
				});
