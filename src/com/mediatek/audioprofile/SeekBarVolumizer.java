...
public class SeekBarVolumizer implements OnSeekBarChangeListener, Handler.Callback {
    ...
    public void setSeekBar(SeekBar seekBar) {
		...
		//+add  by weixiaohui.hct for AudioProfile setting
		if((mStreamType == AudioProfileManager.STREAM_MUSIC)||(mStreamType == AudioProfileManager.STREAM_VOICE_CALL)){
          mOriginalStreamVolume = mSystemVolume;
          Xlog.d(TAG, "ddd initSeekBar sync " + mStreamType + " original Volume to"
             + mSystemVolume);
          } 	
		//-add  by weixiaohui.hct for AudioProfile setting
        mSeekBar = seekBar;
    ...
    public void revertVolume() {
		...
		//+add  by weixiaohui.hct for AudioProfile setting
		if((mStreamType == AudioProfileManager.STREAM_MUSIC)||(mStreamType == AudioProfileManager.STREAM_VOICE_CALL)){
           Xlog.d(TAG, "ddd revertVolume " + mStreamType + " ,: "+ mSystemVolume);
             setVolume(mStreamType, mOriginalStreamVolume, false);
        } 
    ...
    public void saveVolume() {
		...
		if((mStreamType == AudioProfileManager.STREAM_MUSIC)||(mStreamType == AudioProfileManager.STREAM_VOICE_CALL)){
           setVolume(mStreamType, mLastProgress, false);
            Xlog.d(TAG, "ddd saveVolume " + mStreamType + " ,: "+ mLastProgress);
        } 
