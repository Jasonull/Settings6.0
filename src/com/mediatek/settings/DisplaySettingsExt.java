...
public class DisplaySettingsExt implements OnPreferenceClickListener {
    ...
    /* added by hct lijunyi, 2015-05-13 */
    private List<ResolveInfo> mListInfo;
    /* end of added by hct lijunyi, 2015-05-13 */
    ...
    private void initPreference(PreferenceScreen screen) {
        ...
        mMiraVision = createPreference(TYPE_PREFERENCE, R.string.mira_vision_title,
                KEY_MIRA_VISION, mDisplayPerCategory);
        ...
        mWallpaperPref = createPreference(TYPE_PREFERENCE, R.string.wallpaper_settings_title,
                KEY_MTK_WALLPAPER, mDisplayPerCategory);

        /* modified by hct lijunyi, 2015-05-13 */
        Intent intent = new Intent("android.intent.action.HCT_SET_WALLPAPER");
        mListInfo = mContext.getPackageManager().queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);

        if (mListInfo.size() > 0) {
            mWallpaperPref.setIntent(intent);
        } else {
            mWallpaperPref.setFragment("com.android.settings.WallpaperTypeSettings");
        }
        /* end of modified by hct lijunyi, 2015-05-13 */
	...
	//jiashixian  add	00000006# 
    	if (SystemProperties.getBoolean("persist.sys.visitor_mode", false))
        {
		mWallpaperPref.setEnabled(false);
	} else
           {
		mWallpaperPref.setEnabled(true);
         }
// end by jiashixian
        ...
        /* modified by hct lijunyi, 2015-05-13 */
        if (mListInfo.size() > 0) {
            mWallpaperPref.setSummary(null);
        } else {
    ...
    private boolean queryPluginKeyguardLayers() {
        IKeyguardLayer ext;
        ext = (IKeyguardLayer) MPlugin.createInstance(IKeyguardLayer.class.getName(), mContext);
        return ext != null;
    }
