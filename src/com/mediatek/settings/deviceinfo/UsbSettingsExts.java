...
public class UsbSettingsExts {
    ...
    public PreferenceScreen addUsbSettingsItem(UsbSettings usbSettings) {
        ...
        if (context.getResources().getBoolean(R.bool.hct_config_show_usb_ums)) {
        if (umsExist) {
            mUms = new RadioButtonPreference(context);
            ...
            mRadioButtonPreferenceList.add(mUms);
        }
        }
        ...
        if (chargeExist) {
            mCharge = new RadioButtonPreference(context);
        ...
        if (!context.getResources().getBoolean(R.bool.hct_config_remove_cdrom)) {
        if (bicrExist) {
            ...
            mBicr = new RadioButtonPreference(context);
        ...
        Log.d(TAG, "umsExist : " + umsExist + " chargeExist : " + chargeExist
                        + " bicrExist : " + bicrExist);
        return root;
    }
    ...
    public void dealWithBroadcastEvent(Intent intent) {
        ...
        if (UsbManager.ACTION_USB_STATE.equals(action)) {
            ...
            if (plugType == USB_CHARGING_PLUGIN) {
                ...
                if (FeatureOption.MTK_DUAL_INPUT_CHARGER_SUPPORT && plugType == DUAL_INPUT_CHARGER) { //ALPS01957516 can't finish activity if dual charging
                    return;
                }
                mNeedExit = true;
            }
