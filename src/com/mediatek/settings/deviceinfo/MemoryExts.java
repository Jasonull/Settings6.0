...
public class MemoryExts {
    ...
    private static final String TAG_CONFIRM_SWAP = "SwapAlert";  //chenyichong modify for: ALPS02298502
    ...
    public MemoryExts(Activity activity, PreferenceScreen preferenceScreen,
        ...
        Log.d(TAG, "SD SWAP : " + FeatureOption.MTK_2SDCARD_SWAP
    ...
    private OnPreferenceChangeListener installLocationListener = new OnPreferenceChangeListener() {
        ...
        public boolean onPreferenceChange(Preference preference, Object newValue) {
            ...
            Log.d(TAG, "installLocation : " + installLocation);
    ...
    private void updateInstallLocation() {
        ...
        for (RadioButtonPreference volumePreference : mStorageWritePathList) {
            ...
            if (Environment.MEDIA_SHARED.equals(volumeState)) {
                ...
                Log.d(TAG, "Volume state is MEDIA_SHARED, path " + writePath);
                return;
            }
        }
        ...
        boolean enable = !(FeatureOption.MTK_2SDCARD_SWAP && !UtilsExt.isExSdcardInserted());
    ...
    private void updateAPKInstaller() {
        ...
        for (RadioButtonPreference volumeItemPreference : mStorageWritePathList) {
            ...
            String volumeState = mStorageManager.getVolumeState(writePath);
            Log.d(TAG, "Path : " + writePath + " state : " + volumeState);
    ...
    public boolean isInUMSState() {
        for (RadioButtonPreference preference : mStorageWritePathList) {
            ...
            String volumeState = mStorageManager.getVolumeState(writePath);
            if (Environment.MEDIA_SHARED.equals(volumeState)) {
                Log.d(TAG, "Current is UMS state, remove the unmount dialog");
                return true;
            }
        }
        return false;
...
/*
    ...
    public static class SwapAlertFragment extends DialogFragment {
        private static IMountService mMountService;
        
        public static void show(Activity mActivity, IMountService mountService) {
            final SwapAlertFragment dialog = new SwapAlertFragment();
            mMountService = mountService;
            dialog.show(mActivity.getFragmentManager(), TAG_CONFIRM_SWAP);
        }
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            builder.setTitle(R.string.dlg_mount_external_sd_title);
            builder.setMessage(R.string.dlg_mount_external_sd_summary);
    ...
    //chenyichong add static for: ALPS02298502
    private static void doMount(final IMountService mountService) {
        new AsyncTask<Void, Void, Void>() {
            @Override
            protected Void doInBackground(Void... params) {
                try {
                    if (mountService != null) {
                        Log.d(TAG, "Settings mountVolume : " + sClickedMountPoint);
    ...
    public void setUsbEntranceState(UsbManager usbManager, MenuItem usb) {
        boolean isUsbCableInserted = ( usbManager.getCurrentState() != 0 );
